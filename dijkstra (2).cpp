    ...
    while (visited < nodes)
    {
        ...
        while (link_cur != nullptr)
        {
            ...
            if (vis_arr[tmp_end] == false)
            {
                ...
                if (dist_arr[tmp_end] < cmindist)
                {
                    cmindist = dist_arr[tmp_end];
                    cclstnodei = tmp_end;
                }
            }
            link_cur = link_cur->next;
...
void heapify(minh_node arr[], int arr_size, int i)
{
    int l = i;
    int li = 2 * i + 1;
    int ri = 2 * i + 2;

    if (li < arr_size && ((arr[l] < arr[li]) == true))
        l = li;

    if (ri < arr_size && ((arr[l] < arr[ri]) == true))
        l = ri;
    ...
    if (l != i)
    {
        ...
        arr[l] = arr[i];
        arr[i] = tmp;
        heapify(arr, arr_size, l);
    }
}
void heap_sort(minh_node arr[], int arr_size)
{
...
void my_dijkstra5(Graph& g, Node* start_city_node, Node* end_city_node, int mode)
{
    ...
    minh_node* pr = new minh_node[nodes];
    int pr_size = -1;
