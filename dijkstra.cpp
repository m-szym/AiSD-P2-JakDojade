...
//
//    minh_node* pr = new minh_node[10000*nodes];
//    int pr_size = -1;
//
//    for (int i = 0; i < 10000*nodes; i++)
//    {
//        if (i < nodes)
//        {
//            dist_arr[i] = INT_MAX;
...
void my_dijkstra5(Graph& g, Node* start_city_node, Node* end_city_node, int mode)
{
    ...
    //minh_node* pr = new minh_node[10000 * nodes];
    PriorityQueueList2d<minh_node> pr;
    //int pr_size = -1;
    ...
    //enq(pr, pr_size, start_i, 0);
    pr.append(new minh_node(start_i, 0, 0));
    //pr.smart_before_insert(new minh_node(start_i, 0));
    while (visited <= nodes)
    {
        minv = INT_MAX;
        ...
        while (link_cur != nullptr)
        {
            ...
            if (vis_arr[tmp_end] == false)
            {
                ...
                if (tmp_dist < dist_arr[tmp_end])
                {
                    ...
                    //enq(pr, pr_size, tmp_end, tmp_dist);
                    pr.smart_before_insert(new minh_node(tmp_end, tmp_dist, pr.size), visited);
                }
